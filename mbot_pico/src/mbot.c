...
bool timer_cb(repeating_timer_t *rt)
{
    ...
    if (comms_get_topic_data(MBOT_TIMESYNC, &received_time))
    {
         ...
         *************************************************************/
        float delta_d, delta_theta, deltaX, deltaY, theta; // displacement in meters and rotation in radians

        theta = current_odom.theta;
        float dl = enc2meters*current_encoders.left_delta;
        float dr = enc2meters*current_encoders.right_delta;
        delta_theta = (dr - dl)/WHEEL_BASE;
        delta_d = (dr + dl)/2;
        deltaX = delta_d*cos(theta + delta_theta/2);
        deltaY= delta_d*sin(theta + delta_theta/2);
        current_odom.x += deltaX;
        current_odom.y += deltaY;
        ...
        if (comms_get_topic_data(MBOT_MOTOR_COMMAND, &current_cmd))
        {
            ...
            if (OPEN_LOOP)
            {
                ...
                if( linear_vel == 0){
                    ...
                    float left_vel = linear_vel -(b/2)*angular_vel;
                    float right_vel = linear_vel + (b/2)*angular_vel;
                    l_duty= (left_vel - INTERCEPT_L)/SLOPE_L;
                    r_duty= (right_vel - INTERCEPT_R)/SLOPE_R;
                }
            ...
            else
            {
                ...
                float lw_measured = (enc2meters*current_encoders.left_delta/MAIN_LOOP_PERIOD);
                float rw_measured = (enc2meters*current_encoders.right_delta/MAIN_LOOP_PERIOD);
                //Body
                fwd_sp = current_cmd.trans_v;
                turn_sp = current_cmd.angular_v;
                float fwd_measured = (rw_measured + lw_measured)/2;
                float turn_measured = (rw_measured-lw_measured)/WHEEL_BASE;
                float fwd_sp_error = fwd_sp - fwd_measured;
                float turn_sp_error = turn_sp - turn_measured;
                if(fwd_sp == 0){
                    l_duty =0;
                    r_duty =0;
                }
                else{
                    fwd_sp = fwd_sp + rc_filter_march(&fwd_vel_pid, fwd_sp_error);
                }
                turn_sp = turn_sp + rc_filter_march(&turn_vel_pid, turn_sp_error); 
                
                
                //Wheels
                float lsetpoint;
                float rsetpoint;
                 rsetpoint = fwd_sp +(turn_sp*WHEEL_BASE)/2;
                 lsetpoint = fwd_sp -(turn_sp*WHEEL_BASE)/2;
                if (fwd_sp == 0 && turn_sp == 0){
                    l_duty = 0;
                    r_duty = 0;
                ...
                if (  lsetpoint == 0){
                    ...
                    float lerror = lsetpoint - lw_measured;
                    l_duty = (lsetpoint- INTERCEPT_L)/ SLOPE_L + rc_filter_march(&left_pid, lerror);
                ...
                if( rsetpoint ==0){
                    ...
                    float rerror = rsetpoint - rw_measured;
                    r_duty = (rsetpoint- INTERCEPT_R)/SLOPE_R + rc_filter_march(&right_pid, rerror);
                }
