...
slam_data_num = len(slam_data)
odom_data_num = len(odom_data)
# tmp = int(odom_data_num/slam_data_num)+1
# new_odom_data = np.empty((0,2), dtype=float)
# for i in range(slam_data_num):
#     avg = np.zeros((1,2))
#     for j in range(tmp):
#         avg += odom_data[i*tmp+j]
#     avg /= tmp
#     np.append(new_odom_data, avg)
odom_data = np.array_split(odom_data, len(slam_data))
new_odom_data = np.zeros((len(slam_data), 2), dtype=float)
...
for i in range(len(slam_data)):
    ...
    for j in range(len(tmp)):
        new_odom_data[i] += tmp[j]
    new_odom_data[i] /= len(tmp)
...
pose_diff = np.sqrt(np.sum((slam_data-new_odom_data)**2, axis=1))

fig, ax = plt.subplots(1, 1, sharey=True, figsize=(10, 10))
ax.plot(new_time_data, pose_diff, 'b')
# ax.legend()
